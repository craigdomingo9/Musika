        ...
        <div ref={containerRef} className="sm:mt-5 sm:grid sm:grid-cols-[50%_50%] max-w-full">
            <div className="relative">
                ...
                {product.images.length > 1 && (
                    <div className={cn("absolute flex bottom-2 right-1/2")}>
                        {product.images.map((_, id) => (
                            <Circle
                                key={id}
                                className={cn("text-amber-700 w-[0.85rem]", id === current - 1 && "fill-amber-700 opacity-70")}
                            />
                        ))}
                    </div>
                )}
            ...
            <div>
                <div className="flex justify-between w-full px-2 pt-4">
                    ...
                    <div className="flex px-2">
                        <p className={cn("font-bold text-xl", product.on_sale && "line-through text-gray-500")}>
                            {product.price}
                        </p>
                        {product.on_sale && (
                            <p className="font-bold text-xl pl-2">{product.sale_price}</p>
                        )}
                    </div>
                </div>
                        ...
                        <div className="flex">
                            {product.business?.logo ? (
                                <Image
                                    ...
                                    src={`${base_url}${product.business.logo}`}
                                    width={50}
                                    height={50}
                                    alt="Business Logo"
                                />
                ...
                <Link
                    href={{
                        pathname: "/catalog",
                        query: { id: product.catalog.id }
                    }}
                >
                    <div className="w-full flex px-2 py-5 justify-between">
                        <div className="flex w-full">
                            <Button variant="outline" className="m-auto w-full py-3 pl-2 font-bold opacity-85">
                                View more from {product.catalog.name}
                            </Button>
                        </div>
                    </div>
                </Link>
