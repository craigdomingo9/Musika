...
function AnalyticsCompareChartListAdd({ productAnalytics }: Props) {
    ...
    const addInstanceToStore = (instance: ProductAnalytics) => {
        addInstance(instance);
        toggleSecondaryActionOccurred(!secondaryActionOccured);
    };

    const removeInstanceFromStore = (product: number) => {
    ...
    }, [secondaryActionOccured, productAnalytics]);
                ...
                <DialogContent className="sm:max-w-[425px] grid">
                    ...
                    <DialogDescription>
                        ...
                        <span className='block'>
                            {instances.map((instance: ProductAnalytics) => (
                                <Button 
                                    key={instance.product} 
                                    variant={"outline"} 
                                    onClick={() => removeInstanceFromStore(instance.product)} 
                    ...
                    {productAnalytics.map((analytics_instance: ProductAnalytics, index) => (
                        !instances.find(instance => instance.product === analytics_instance.product) && (
                            <div key={analytics_instance.product} className='grid grid-cols-[85%_15%]'>
                                ...
                                <div className='grid h-full place-items-center mx-2'>
                                    <Button 
                                        variant={"secondary"} 
                                        onClick={() => addInstanceToStore(analytics_instance)}  
                                        className='h-14'
                                        aria-label={`Add ${analytics_instance.product_details.name} to compare`}
                                    >
                                        <PlusSquareIcon />
                                    </Button>
                                </div>
