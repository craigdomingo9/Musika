...
function InventoryProductItemEditFormDeleteImageButton({product,index}: Props) {
    ...
    async function deleteImage(){
        if(await productDelete(product.images[index].id.toString(),true,router)){
            toast({
                variant: "green",
    ...
    <div className='grid'>
        <AlertDialog>
            <AlertDialogTrigger className='mx-auto'>
                <Button variant={"outline"} type="button" className="mx-auto flex w-36 h-8 rounded-none text-red-600">
...
export default InventoryProductItemEditFormDeleteImageButton