...
function InventoryDeleteCatalog({ catalog }: Props) {
    ...
    async function deleteCatalog() {
        try {
            const success = await catalogDelete(catalog);
            if (success) {
                toast({
                    variant: "success", // Change to a defined variant
                ...
                toast({
                    variant: "destructive", // Use a variant for errors
                    description: "Failed to delete the catalog. Please try again.",
                    duration: 3000,
                });
            }
        } catch (error) {
            toast({
                variant: "destructive",
                description: "An error occurred while deleting the catalog.",
                duration: 3000,
            });
        }
    }
